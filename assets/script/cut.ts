...
export class cut extends Component {
    ...
    private lineGraphics = null // 划动轨迹的绘制组件
    ...
    onTouchMove(event) {
        ...
        if (deltaX > 50 || deltaX < -50) { // 处理左右位移过大时不生成轨迹效果
            if (this.lineGraphics) {
                this.lineGraphics.clear()
                this.lineNode.removeFromParent()
            }
        ...
        if (!this.lineGraphics) { // 如果绘制组件为空，创建一个
            this.lineGraphics = this.lineNode.addComponent(Graphics)
        }
        this.lineGraphics.lineWidth = 10 // 设置轨迹的宽度
        this.lineGraphics.strokeColor = color(0, 0, 0, 50) // 设置轨迹颜色以及不透明度
        const uiTransform = this.node.getComponent(UITransform)
        let startPoint = uiTransform.convertToNodeSpaceAR(this.startPos) // 节点空间转化
        let endPoint = uiTransform.convertToNodeSpaceAR(pos) // 节点空间转化
        this.lineGraphics.moveTo(startPoint.x, startPoint.y) // 移动到起始点
        this.lineGraphics.lineTo(endPoint.x, endPoint.y) // 划动到结束点
        this.startPos = pos
    }
    ...
    onLoad() {
        console.log('cut')
        this.node.on(Node.EventType.TOUCH_START, ()=>{console.log('TOUCH_START')}, this)
